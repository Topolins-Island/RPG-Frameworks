...
public class PlayerUnitController : MonoBehaviour
{
    ...
    public void ShowMyStuff(PlayerUnit me)
    {
        ...
        if (current == null)
        {
            if (!moveHolder.activeInHierarchy)
    ...
    void StartPlayer()
    {
        ...
        for (int i = 0; i < moveButtons.Count; i++)
        {
            if (i < current.moves.Count)
            {
                ...
                moveButtons[i].GetComponentInChildren<Text>().text = current.moves[i].name;
        ...
        BattleManager.instance.DisplayMessage("What will " + current.name + " do?");
